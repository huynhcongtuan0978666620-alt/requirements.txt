                ...
                for r in tam_records:
                    if (
                        ...
                        if new_gio:
                            ...
                            st.toast(
                                "🔄 Đã tự động khôi phục giỏ hàng làm dở trước đó!"
                            )
                        break
                    ...
                    if len(lh_data) > 1:
                        ...
                        c_tt_lh = next(
                            (
                                c
                                for c in df_lh.columns
                                if "trạng thái" in c.lower()
                                or "trang thai" in c.lower()
                            ),
                            None,
                        )
                        c_tho_lh = next(
            ...
            with tabs[1]:
                ...
                with st.container(border=True):
                    ...
                    if st.button(
                        ...
                        if dv_chon:
                            for dv in dv_chon:
                                info_dv = services.get(
                                    dv, {"gia": 0.0, "hoa_hong": 0.0}
                                )
                                existing = next(
                                    (
                                        item
                ...
                if st.session_state.gio_hang:
                    with st.container(border=True):
                        ...
                        max_sl = (
                            2.0
                            if st.session_state.get("role") == "NhanVien"
                            else 1000.0
                        )
                        ...
                        if st.button(
                            "💾 LƯU ĐƠN VÀO BILL CHỜ", use_container_width=True
                        ):
                            if luu_bill_tam(
...
<div style="text-align: center; margin-top: 15px; font-size: 13px; color: {THEME_COLORS['text_muted']}; font-style: italic;">Cảm ơn quý khách đã sử dụng dịch vụ!</div>
                ...
                with st.container(border=True):
                    ...
                    if st.button(
                        ...
                        if hen_ten and hen_sdt:
                            try:
                                ws_hen = get_google_sheet_workbook().worksheet(
                                    "LichHen"
                                )
                                ws_hen.append_row(
                                    [
                                ...
                                with c_btn:
                                    if tt_val.strip().upper() == "ĐÃ CHỐT ĐƠN":
                                        ...
                                        if st.button(
                                            ...
                                            try:
                                                col_idx = (
                                                    df_lh_full.columns.get_loc(c_tt_lh)
                                                    + 1
                                                    if c_tt_lh
                                                    else 7
                                                )
            ...
            with tabs[3]:
                if st.session_state["role"] == "Admin":
                    with st.container(border=True):
                        ...
                        try:
                            ...
                            if len(data_bc) > 1:
                                ...
                                df_month = (
                                    df_bc[
                                        df_bc[c_ngay].str.contains(month_str, na=False)
                                    ]
                                    if c_ngay in df_bc.columns
                                    else pd.DataFrame()
                                )
                                ...
                                m4.metric(
                                    "👥 KHÁCH ĐÃ PHỤC VỤ HÔM NAY",
                                    f"{khach_hom_nay} Khách",
                                )
                                m5.metric(
                                    "⏳ TỔNG ĐƠN CHỜ HIỆN TẠI", f"{bill_cho} Bill"
                                )
                    ...
                    st.warning(
                        "🔒 Chức năng này chỉ dành cho tài khoản có quyền Quản lý."
                    )
                    ...
                    with st.container(border=True):
                        ...
                        if st.button(
                            ...
                            if "Vàng Đồng" in mau_nhuom:
                                ...
                                loi_khuyen = (
                                    "Theo dõi sát biểu bì tóc để xả nước kịp thời."
                                )
                                diem_so = "8.5/10"
