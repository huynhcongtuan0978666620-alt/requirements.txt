...
def tinh_hoa_hong_tho(df_bao_cao):
    ...
    if col_hh not in df_bao_cao.columns:
        ...
        col_hh = next(
            (
                c
                for c in df_bao_cao.columns
                if "thợ" in c.lower() and ("%" in c or "‰" in c)
            ),
            None,
        )
    ...
    col_tho = next(
        (
            c
            for c in df_bao_cao.columns
            if "thợ" in c.lower() and "tiền" not in c.lower()
        ),
        "Thợ",
    )
    ...
    if col_hh and col_hh in df_bao_cao.columns:
        df_bao_cao[col_hh] = pd.to_numeric(df_bao_cao[col_hh], errors="coerce").fillna(
            0
        )
                            ...
                            for i, row in df_today_lh.iterrows():
                                ...
                                if tt_val.strip().upper() == "ĐÃ CHỐT ĐƠN":
                                    ...
                                    if st.button(
                                        ...
                                        trigger_auto_save()
                                        st.toast(
                                            "✅ Đã nạp thành công! Vui lòng nhấp sang Tab Lên Hóa Đơn."
                                        )
                                st.markdown(
...
# --- KPI THỢ HÔM NAY (V16 NÂNG CẤP) ---
                            ...
                            if not df_today.empty:
                                ...
                                # 3. Xác định cột Thợ
                                col_tho_name = "Thợ phụ trách" if "Thợ phụ trách" in df_today.columns else next(
                                    (c for c in df_today.columns if ("thợ" in c.lower() or "tho" in c.lower() or "thực hiện" in c.lower()) and "tiền" not in c.lower()), None
                                )

                                if col_tho_name:
                                    # Chuyển kiểu dữ liệu sang số để tính toán
                                    df_today[c_tien] = pd.to_numeric(df_today[c_tien], errors="coerce").fillna(0)
                                    df_today["Tiền Công Thợ (%)"] = pd.to_numeric(df_today["Tiền Công Thợ (%)"], errors="coerce").fillna(0)
                                    
                                    # Tính tiền hoa hồng cho từng đơn
                                    df_today["HH_Tung_Don"] = df_today[c_tien] * (df_today["Tiền Công Thợ (%)"] / 100)
                                    ...
                                    st.warning("Hệ thống chưa dò thấy cột thông tin Thợ trong dữ liệu.")
                            else:
                                st.info("Hôm nay chưa có dữ liệu giao dịch hoàn thành để tính KPI.")
