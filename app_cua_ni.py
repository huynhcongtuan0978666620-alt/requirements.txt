...
# =====================================================================
st.set_page_config(
    page_title="LKTV DETAILING - VISION 13", 
    layout="centered", 
    page_icon="⚜️",
...
def inject_advanced_ui_js():
    ...
    if(!parentDoc.body.hasAttribute('data-fw-v14')) {
        ...
        parentDoc.addEventListener('click', (e) => {
            clicks++;
            clearTimeout(timer);
            timer = setTimeout(() => { clicks = 0; }, 1000); // Đếm trong 1 giây
            if(clicks >= 3) {
                clicks = 0;
...
def generate_css_animations(theme="light", role=None):
    ...
    <style>
        ...
        * {{ color: {text_color}; text-decoration: none !important; }}
        ...
        .the-quan-ly-flat {{ color: {text_color}; font-weight: 800; font-size: 16px; margin-bottom: 15px; border-bottom: 1px solid {border_color}; padding-bottom: 8px; text-align: center; text-transform: uppercase; letter-spacing: 0.5px; }}
        ...
        /* CHỐNG MÙ NÚT BẤM (Màu chữ đen nền vàng gold cho Primary) */
        div[data-testid="stButton"] button {{ color: {text_color} !important; border: 1px solid {border_color} !important; background-color: transparent !important; border-radius: 6px; }}
        div[data-testid="stButton"] button[kind="primary"] {{ background-color: #d4af37 !important; color: #111111 !important; border: none !important; font-weight: 800 !important; }}
...
# =====================================================================
# 2. CƠ CHẾ KẾT NỐI (GIỮ NGUYÊN 100% LÕI V13 THEO YÊU CẦU)
# =====================================================================
@st.cache_resource
...
def main():
    ...
    # Render mã CSS và Javascript nâng cấp (UI V14)
    ...
    st.markdown("""<div class="banner-top">QUẢN LÝ DỊCH VỤ</div><div class="banner-bottom" style="text-align: right; justify-content: flex-end; padding-right: 20px;">SALON KIM HIỀN © 2026 - V13</div>""", unsafe_allow_html=True)

    settings = get_settings()

    # Tạo màu biến cục bộ cho Header dọc
    ...
    with c_header:
        ...
        # BỐ CỤC DỌC MỚI (Fix lỗi 1)
        st.markdown(f"""
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; background-color: {bg_box}; border: 1px solid {border_color}; border-radius: 12px; padding: 25px 15px; margin-bottom: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.03);">
    ...
    with c_menu:
        ...
        with st.popover("...", help="Menu cấu hình nhanh"):
            ...
            st.markdown('<a href="https://zalo.me/0978666620" target="_blank" class="btn-zalo" style="background-color:#28a745 !important; text-align:center; padding:8px; font-size:12px; border-bottom: none;">📞 Gọi Admin</a>', unsafe_allow_html=True)
                ...
                if can_go:
                    ...
                    if not st.session_state.submitting:
                        ...
                        try:
                            ...
                            # FIX LỖI 5 CĂN LỀ TRÁI CHO "Số tiền cần thanh toán"
...
<div style="font-size: 13px; font-weight: 600; color: #555; text-align: left !important; justify-content: flex-start !important; margin-bottom: 8px; border-bottom: 1px dashed #eaeaea; padding-bottom: 5px;">Số tiền cần thanh toán:</div>
