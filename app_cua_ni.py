...
# ==========================================
# 1. GIAO DIỆN CHUẨN LKTV - KHÓA NÚT CHỐNG TRÙNG
# ==========================================
    ...
    if not st.session_state["logged_in"]:
        ...
        if st.session_state["role"] == "Admin":
            with tabs[1]:
                ...
                if st.button("🔄 Cập nhật dữ liệu mới"): st.rerun()
                try:
                    client = get_gspread_client()
                    sh = client.open_by_url(st.secrets["connections"]["gsheets"]["spreadsheet"])
            ...
            with tabs[2]:
                ...
                with st.expander("🔑 TRÌNH TẠO MÃ BẢO MẬT"):
        ...
        if st.sidebar.button("🚪 Đăng xuất"):
            st.session_state.update({"logged_in": False, "submitting": False})
            st.rerun()
