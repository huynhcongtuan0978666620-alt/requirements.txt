...
# --- KPI THỢ HÔM NAY (V16 NÂNG CẤP) ---
                            ...
                            if not df_today.empty:
                                try:
                                    ...
                                    # LÀM SẠCH DỮ LIỆU ĐỂ GHÉP BẢNG (Quan trọng)
                                    df_today['Dịch vụ'] = df_today['Dịch vụ'].astype(str).str.strip()
                                    df_dm['Tên Sản Phẩm'] = df_dm['Tên Sản Phẩm'].astype(str).str.strip()
                                    
                                    # Ép kiểu dữ liệu số
                                    df_dm['Tiền Công Thợ (%)'] = pd.to_numeric(df_dm['Tiền Công Thợ (%)'], errors='coerce').fillna(0)
