...
def main():
    init_states = {
        ...
        "bill_vua_in": None, "kh_sdt_val": "", "kh_ten_val": "", 
    ...
    # --- KHU VỰC LÀM VIỆC CHÍNH ---
        ...
        tabs = st.tabs(["🏠 Tổng quan", "📄 Lên hóa đơn", "📅 Lịch hẹn", "📊 Báo cáo", "⚙️ Quản trị & Mở rộng"])
        services = get_service_data()
                ...
                try:
                    ...
                    if len(bc_values) > 1:
                        ...
                        if not last_7_days.empty:
                            st.markdown(f"<div style='margin-top:25px; margin-bottom:10px; font-weight:bold; font-size:15px; color:{THEME_COLORS['text_title']}; letter-spacing:0.5px;'>📉 TĂNG TRƯỞNG DOANH THU 7 NGÀY QUA</div>", unsafe_allow_html=True)
                        ...
                        if not df_today_lh.empty:
                            for i, row in df_today_lh.iterrows():
                                ...
                                if tt_val.strip().upper() == "ĐÃ CHỐT ĐƠN":
                                    st.markdown(f"<div style='text-align:right; font-weight:bold; font-size:14px; color:#28a745; margin-bottom:10px;'>✅ ĐÃ CHỐT ĐƠN</div>", unsafe_allow_html=True)
                                    ...
                                    if st.button("📝 Nạp & Lên Đơn", key=f"lh_home_{idx_sheet}", use_container_width=True):
                                        st.session_state.kh_sdt_val = str(sdt)
                                        st.session_state.kh_ten_val = str(khach)
                                        st.session_state.tho_chot_val = str(tho_yc) if tho_yc != "Không yêu cầu" else st.session_state.full_name
                                        st.session_state.current_lh_idx = idx_sheet
                                        try:
                                        ...
                                        st.toast("✅ Đã nạp thông tin. Chuyển sang Tab Lên Hóa Đơn nhé!")
                                        time.sleep(0.5)
                                        st.rerun()
                                st.markdown("<hr style='margin:8px 0; border:none;'>", unsafe_allow_html=True)
                        if not has_items: st.info("Không có lịch hẹn nào của bạn cho hôm nay.")
        ...
        with tabs[1]:
            ...
            with st.container(border=True):
                ...
                dv_chon = st.multiselect("Chạm chọn dịch vụ...", options=dv_list if dv_list else ["Đang tải..."], max_selections=max_dv, placeholder=pholder_text, key=f"dv_{st.session_state.reset_counter}")
                ...
                with st.container(border=True):
                    ...
                    for idx, item in enumerate(st.session_state.gio_hang):
                        ...
                        with c_del:
                            ...
                            if st.button("🗑️ Xoá dịch vụ", key=f"del_{idx}", use_container_width=True):
            ...
            if t_bill > 0:
                with st.container(border=True):
                    ...
                    try: default_idx = opts_tho.index(st.session_state.tho_chot_val)
                    ...
                    st.markdown(f"""
                        <div style="background-color: #eaf2e6; border: 1px solid #87e8de; border-left: 5px solid #13c2c2; padding: 12px; border-radius: 10px; text-align: left; margin: 15px 0 5px 0; font-weight: 700; color: #111; font-size: 14px;">
                            👉 XÁC NHẬN SỐ TIỀN THỰC THU: <br>
                            <span style="color:#cf1322; font-size:16px;">{t_khach_tra:,.0f}đ</span>
                            <span style="font-weight:400; font-size:12px; color:#555;">(Hệ thống đang kiểm tra....)</span>
                        </div>
                    """, unsafe_allow_html=True)
                            ...
                            with c_btn:
                                if tt_val.strip().upper() == "ĐÃ CHỐT ĐƠN":
                                    ...
                                    if st.button("📝 Lên Đơn", key=f"lh_tab3_{idx_sheet}", use_container_width=True):
                                        ...
                                        if dv:
                                            for d_v in dv.split(", "):
                                                ...
                                                if d_v_clean in services:
                                                    ...
                                                    new_gio.append({
                                                        "dich_vu": d_v_clean, "so_luong": 1.0,
                                                        "don_gia": inf["gia"], "thanh_tien": inf["gia"],
                                        ...
                                        trigger_auto_save()
                                        st.toast("⚡ Chuyển dữ liệu lịch hẹn ra giỏ hàng thành công!")
                                        time.sleep(0.5)
                                        st.rerun()
            ...
            if st.session_state["role"] == "Admin":
                                            ...
                                            if st.button("🛒 Nạp ra giỏ", key=f"load_{sheet_row_idx}", use_container_width=True, type="primary"):
                                                with st.spinner("Đang nạp..."):
                                                    ...
                                                    if xoa_bill_tam_dong_goc(sheet_row_idx):
                                                        st.toast("⚡ Đã nạp đơn chờ vào giỏ hàng thành công!")
                                                        time.sleep(0.5)
                                                        st.rerun()
                            if not has_pending: st.info("Không có đơn chờ duyệt nào hiện tại.")
                        else: st.info("Không có đơn chờ duyệt nào hiện tại.")
                ...
                with st.container(border=True):
                    ...
                    if st.button("⚗️ THỬ NGHIỆM CÔNG THỨC & TỶ LỆ", type="primary", use_container_width=True):
                        ...
                        st.markdown(f"<h4 style='color:{THEME_COLORS['text_title']}; margin-top: 15px;'>2. Quy trình pha chế / bôi thuốc chuẩn</h4>", unsafe_allow_html=True)
                        st.markdown(f"""
                        ...
                        * **Bước 4:** Để thời gian lưu thuốc ổn định trên tóc từ 35-45 phút tùy thuộc vào độ thẩm thấu của sợi tóc.
                        ...
                        st.markdown(f"<h4 style='color:{THEME_COLORS['text_title']}; margin-top: 15px;'>4. Hướng dẫn thao tác / Hướng dẫn sử dụng chi tiết</h4>", unsafe_allow_html=True)
                ...
                with st.container(border=True):
                    ...
                    if st.button("♻️ LÀM MỚI TOÀN BỘ BỘ NHỚ ĐỆM", use_container_width=True, type="primary"):
                        st.cache_data.clear()
