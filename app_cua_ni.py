...
# --- KPI THỢ HÔM NAY (V16 NÂNG CẤP) ---
                            ...
                            if not df_today.empty:
                                try:
                                    # NÍ SỬA CHỖ NÀY: 
                                    # Nếu ní dùng gspread: dùng gc.open("Tên_File").worksheet("DanhMuc").get_all_values()
                                    # Nếu ní dùng st.connection: dùng conn.read(worksheet="DanhMuc")
                                    # Ở đây tôi đang giả định ní dùng 'gc' để mở file (ní kiểm tra đầu file của ní nhé)
                                    rows = gc.open("BC_DULIEU_DEMO_2026").worksheet("DanhMuc").get_all_values()
